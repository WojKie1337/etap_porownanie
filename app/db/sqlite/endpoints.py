...
from app.db.sqlite.database import get_db
...
# pobierz wszystkie
# dodanie paginacji
@router.get("/", tags=["SQLite"], response_model=list[schemas.NoteResponse])
...
# pobierz jeden
@router.get("/{id}", tags=["SQLite"], response_model=schemas.NoteResponse)
...
# dodaj jeden
@router.post("/", status_code=201, tags=["SQLite"], response_model=schemas.NoteResponse)
...
# usuń jeden
@router.delete("/{id}", status_code=204, tags=["SQLite"])
