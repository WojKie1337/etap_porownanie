...
# pobierz wszystkie
# dodanie paginacji
@router.get("/", tags=["MySQL"], response_model=list[schemas.NoteResponse])
...
# pobierz jeden
@router.get("/{id}", tags=["MySQL"], response_model=schemas.NoteResponse)
...
# dodaj jeden
@router.post("/", status_code=201, tags=["MySQL"], response_model=schemas.NoteResponse)
...
# usuń jeden
@router.delete("/{id}", status_code=204, tags=["MySQL"])
